...
def plot_dm(type='mean', extra_exp_flag=False):
  plotter = Plotter()
  if extra_exp_flag:
    games = [
    ...
    titles = [
        'Hopper', 'Acrobot', 'Finger', 'Humanoid-stand-walk',
        'Humanoid-stand-run', 'Manipulator', 'Quadruped', 'Stacker', 'Swimmer'
    ]
    ...
    games = [
        'dm-cartpole-b', 'dm-reacher', 'dm-cheetah', 'dm-fish', 'dm-walker-1',
        'dm-walker-2'
    ]

    titles = ['CartPole', 'Reacher', 'Cheetah', 'Fish', 'Walker1', 'Walker2']

    patterns = [
        'remark_PPO-', 'ASC-PPO', 'AHP', 'remark_PPOC',
        'num_workers_4-remark_IOPG', 'nhead1_dm'
    ]

    labels = ['PPO', 'DAC+PPO', 'AHP+PPO', 'PPOC', 'OC', 'SA+PPO']
  ...
  if extra_exp_flag:
    root = './transfer_sa_extra_tf_log/'
  else:
    root = './transfer_tf_log/'
  plot_games(
      plotter,
      games=games,
      patterns=patterns,
...
if __name__ == '__main__':
  mkdir('images')
  plot_dm(type='mean', extra_exp_flag=False)
