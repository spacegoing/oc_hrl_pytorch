...
class SingleOptionNet(nn.Module, BaseNet):
...
class DoeDecoderFFN(nn.Module, BaseNet):
...
class NoTermHead(nn.Module, BaseNet):
  ...
  def __init__(self,
    ...
    self.po_norm = nn.LayerNorm(state_dim + 1)
  ...
  def forward(self, obs, prev_option):
    '''
    Params:
        obs: [num_workers, state_dim]
    ...
    if prev_option.dim() ==1:
      prev_option = prev_option.unsqueeze(1)

    phi_o = self.po_norm(torch.cat([phi, prev_option], dim=-1))
    phi_a = self.actor_body(phi_o)
    phi_a = self.fc_pi_o(phi_a)
    pi_o = F.softmax(phi_a, dim=-1)
    log_pi_o = F.log_softmax(phi_a, dim=-1)
    pi_hat = self.init_po_ffn(phi)
